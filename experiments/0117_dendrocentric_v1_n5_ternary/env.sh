...
export RUN_ID="0117_dendrocentric_v1_n5_ternary"
...
# 0117: Boahen-inspired dendrocentric layer v1 (sparse-selection only; no order
# sensitivity, deferred to v2). Replaces SwiGLU MLP in each block with a
# dendrite bank: M=2048 dendrites, K=8 sparse selection per dendrite via
# top-K STE, sigmoidal NMDA-like nonlinearity, ternary BitLinear output.
# Math + toys verified in scratch/2026-04-29_dendrocentric_v1_*.{md,py}.
# Design choices doc: scratch/2026-04-29_dendrocentric_design_choices.md
# Default DENDROCENTRIC=0 keeps behavior byte-identical to 0107.
export DENDROCENTRIC=1
export DENDRO_M=2048
export DENDRO_K=8
export DENDRO_ALPHA=1.0
