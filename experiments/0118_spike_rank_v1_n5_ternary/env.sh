...
export RUN_ID="0118_spike_rank_v1_n5_ternary"
...
# 0118: spike-rank embedding v1 (sparse-only / variant A). Replace nn.Embedding +
# tied lm_head with a K-sparse representation: each token's row has K=8 nonzero
# values via top-K STE. Saves ~488 KB on embed cap. Tests brief option (c).
# Math + 5 progressive toys verified in scratch/2026-04-29_spike_rank_*.{md,py}.
# Default SPIKE_RANK_EMBED=0 keeps behavior byte-identical to 0107.
export SPIKE_RANK_EMBED=1
export SPIKE_RANK_K=8
