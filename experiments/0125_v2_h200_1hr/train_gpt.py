...
class Hyperparameters:
    ...
    # 0120 dendrocentric layer v2: K-sparse selection + DFSM stored ordering +
    # soft-rank correlation. When DENDROCENTRIC=0, byte-identical to parent 0107.
    # When =1, each Block's MLP is replaced by DendrocentricLayer (v2 forward
    # with stored permutation L per dendrite, soft-rank Pearson correlation,
    # sigmoid NMDA activation, ternary BitLinear out). Math:
    # scratch/2026-04-30_dendrocentric_v2_derivation.md.
    dendrocentric = int(os.environ.get("DENDROCENTRIC", "0"))
    dendro_m = int(os.environ.get("DENDRO_M", "2048"))
    dendro_k = int(os.environ.get("DENDRO_K", "8"))
    dendro_alpha = float(os.environ.get("DENDRO_ALPHA", "4.0"))
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        # Manual GQA expansion (torch < 2.5 lacks enable_gqa kwarg).
...
class Block(nn.Module):
    def __init__(
        ...
        # 0117: optional dendrocentric MLP replacement. Default
        # Hyperparameters.dendrocentric == 0 keeps the original
        # `self.mlp = MLP(dim, mlp_mult)` path → byte-identical to 0107.
        if Hyperparameters.dendrocentric == 1:
            from modules.dendrocentric import DendrocentricLayer
            self.mlp = DendrocentricLayer(
                d_model=dim,
                n_dendrites=Hyperparameters.dendro_m,
                k_per_dendrite=Hyperparameters.dendro_k,
                alpha=Hyperparameters.dendro_alpha,
                tau_x=Hyperparameters.dendro_tau_x,
                tau_l=Hyperparameters.dendro_tau_l,
            )
        else:
