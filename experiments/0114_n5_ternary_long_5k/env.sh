...
export RUN_ID="0114_n5_ternary_long_5k"
...
# 0114: long-train on the cap-frontier setpoint (n=5 ternary). After 0111 (n=7)
# under-trained at 1k steps and 0112 confirmed the SSD scan is load-bearing
# (-0.046 BPB), the right long-train target is the n=5 ternary stack at 5x
# more steps. Tests TWO things at once:
#   (a) BitNet 25x training-closure claim: ternary penalty narrowing at scale
#   (b) val_bpb scaling slope at production batch on the SUBMITTABLE artifact
# 5000 steps × 131072 = 655M tokens trained (~6.5% of fineweb10B). Predicted
# step time 1345 ms, total wallclock ~112 min ($1.87 at $1/hr).
# Predicted val: -0.10 to -0.15 BPB from 1k (1.5232 mean) -> 1.40-1.43 range.
# Submittable artifact ~9 MB unchanged (more training doesn't change weights size).
export ITERATIONS=5000
# WARMDOWN_ITERS scales with ITERATIONS — keep 75% schedule.
export WARMDOWN_ITERS=750
# Wallclock 2.5 hour cap (preflight rejects 0). 1.87h predicted + buffer.
