...
export RUN_ID="0115_parallel_resid_n5_ternary"
...
# 0115: append "presid_" so the new parallel-residual routing scalars
# (presid_routing — substring "presid_") stay fp32 in training and at quant export.
export CONTROL_TENSOR_NAME_PATTERNS="attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,A_log,A_im,B_proj,C_proj,dt_log,D_skip,dt_bias,delta_bias,conv1d,presid_"
...
# ported from KellerJordan/modded-nanogpt PR #230). Maintains TWO residual streams
# (x_attn, x_mlp) starting from PARALLEL_START_LAYER; each partitioned virtual layer
# learns 4 routing scalars (attn_to_attn, attn_to_mlp, mlp_to_attn, mlp_to_mlp).
# Init (1, 0, 0, 1) makes the partitioned forward path byte-identical to single-
# stream at step 0 (modulo the final x_attn + x_mlp merge). Default PARALLEL_RESIDUAL=0
# is byte-identical to parent 0107.
#
# PARALLEL_START_LAYER pins where the network splits into x_attn + x_mlp streams.
# Records used layer 7 of 11 (~64% through). Our config (NUM_UNIQUE_LAYERS=5,
# NUM_LOOPS=3) has 15 virtual layers; the prompt-specified default for this
# config is 10 (yields 5 partitioned virtual layers ⇒ 5 × 4 = 20 new fp32
# routing scalars; ~80 bytes; negligible cap impact).
# The general formula in train_gpt.py (when this env var is empty) is
#     max(1, int(0.64 * NUM_UNIQUE_LAYERS * NUM_LOOPS))
# which would yield 9 here; we set 10 explicitly to match the prompt spec.
export PARALLEL_RESIDUAL=1
export PARALLEL_START_LAYER=10

# Wallclock cap (preflight requires non-zero). Predicted ~22 min on the target
# CUDA pod given parent 0107's step time + the small overhead from the two-stream
# splits in the deeper ~33% of layers. 1800 s = 30 min cushion.
