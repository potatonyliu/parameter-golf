...
class Hyperparameters:
    ...
    # 0115 PARALLEL RESIDUAL (records 2026-03-31_ParallelResiduals_MiniDepthRecurrence,
    # ported from KellerJordan/modded-nanogpt PR #230). Starting at virtual layer
    # `parallel_start_layer`, the network maintains TWO residual streams: attn reads
    # from x_attn, MLP reads from x_mlp, and each sublayer learns 4 routing scalars
    # (attn_to_attn, attn_to_mlp, mlp_to_attn, mlp_to_mlp) that mix outputs back into
    # both streams. Init: attn_to_attn=mlp_to_mlp=1, attn_to_mlp=mlp_to_attn=0 (so
    # before training the path is byte-identical to non-partitioned).
    # Default PARALLEL_RESIDUAL=0 → byte-identical to parent 0107 (no new ops, no
    # parameters consumed).
    # Default PARALLEL_START_LAYER (when env unset / empty) is computed as
    #   max(1, int(0.64 * num_unique_layers * num_loops))
    # matching records' "starting from layer 7 of 11" (~64% through the stack).
    parallel_residual = int(os.environ.get("PARALLEL_RESIDUAL", 0))
    _parallel_start_layer_str = os.environ.get("PARALLEL_START_LAYER", "")
    if _parallel_start_layer_str.strip():
        parallel_start_layer = int(_parallel_start_layer_str)
    else:
        # General formula for any (NUM_UNIQUE_LAYERS, NUM_LOOPS) combo.
        _total_virtual = max(1, num_unique_layers * num_loops)
        parallel_start_layer = max(1, int(0.64 * _total_virtual))
...
class Block(nn.Module):
    ...
    def _attn_substep(self, x: Tensor, x0: Tensor) -> tuple[Tensor, Tensor]:
        """Parallel-residuals helper: run resid_mix + attn_norm + (parallel) attn,
        return (x_after_resid_mix, scaled_attn_contribution_to_residual). The
        caller routes the scaled contribution into one or both residual streams.
        Mirrors the single-stream forward up to (but not including) `x = x + ...`.
        """
        mix = self.resid_mix.to(dtype=x.dtype)
        ...
        if self.parallel_mode:
            ...
            attn_out = self.attn(normed)
            contrib = self.attn_scale.to(dtype=x.dtype)[None, None, :] * attn_out
        return x_mixed, contrib

    def _mlp_substep(self, x: Tensor) -> Tensor:
        """Parallel-residuals helper: run mlp_norm + mlp, return the scaled MLP
        contribution. Caller routes into one or both residual streams.
        """
        return self.mlp_scale.to(dtype=x.dtype)[None, None, :] * self.mlp(self.mlp_norm(x))
...
class GPT(nn.Module):
    def __init__(
        ...
        # 0115 PARALLEL RESIDUAL (records 2026-03-31). When parallel_residual=0
        # (default) we set self.parallel_residual=False and never enter the
        # two-stream code path → byte-identical to parent 0107.
        # When =1, allocate per-virtual-layer routing scalars for every virtual
        # layer index >= parallel_start_layer. Routing layout per partitioned
        # virtual layer:
        #   index 0: attn_to_attn (init 1.0)
        #   index 1: attn_to_mlp  (init 0.0)
        #   index 2: mlp_to_attn  (init 0.0)
        #   index 3: mlp_to_mlp   (init 1.0)
        # The init makes the partitioned forward path byte-identical to the
        # standard single-stream path AT INIT (because attn writes 1·out into
        # x_attn + 0·out into x_mlp, MLP writes 0·out into x_attn + 1·out into
        # x_mlp; merging x_attn + x_mlp at the end recovers the standard sum).
        # Naming: tensor name contains the substring "presid_" so it is matched
        # by CONTROL_TENSOR_NAME_PATTERNS (env in env.sh adds "presid_") and stays
        # fp32 in training and at quant export.
        self.parallel_residual = bool(parallel_residual) and self.recurrent
        if self.parallel_residual:
            total_virtual = num_unique_layers * num_loops
            # Clamp start layer into [1, total_virtual]; if clamp leaves no
            # partitioned layers, silently revert to single-stream mode.
            self.parallel_start_layer = max(1, min(parallel_start_layer, total_virtual))
            num_partitioned = total_virtual - self.parallel_start_layer
            if num_partitioned <= 0:
                self.parallel_residual = False
                self.presid_routing = None
            else:
                # Shape (num_partitioned, 4) — fp32 always (ndim<2 false here so
                # we rely on the substring "presid_" in CONTROL_TENSOR_NAME_PATTERNS
                # for fp32 restoration; restore_low_dim_params_to_fp32 handles it).
                init = torch.zeros(num_partitioned, 4, dtype=torch.float32)
                init[:, 0] = 1.0  # presid_attn_to_attn
                init[:, 1] = 0.0  # presid_attn_to_mlp
                init[:, 2] = 0.0  # presid_mlp_to_attn
                init[:, 3] = 1.0  # presid_mlp_to_mlp
                self.presid_routing = nn.Parameter(init)
                print(
                    f"[GPT] Parallel residuals ENABLED: total_virtual={total_virtual}, "
                    f"parallel_start_layer={self.parallel_start_layer}, "
                    f"num_partitioned_virtual_layers={num_partitioned}, "
                    f"new fp32 scalars={num_partitioned * 4}."
                )
        else:
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.recurrent:
            ...
            if self.parallel_residual:
                # 0115 PARALLEL RESIDUALS: virtual-layer indexing across the
                # num_loops × num_unique_layers nested loop. For virtual indices
                # below parallel_start_layer the standard single-stream path
                # runs (x_attn == x_mlp == x). At the partition boundary we
                # split: x_attn = x_mlp = x. From then on, attn reads from
                # x_attn / MLP reads from x_mlp, and 4 routing scalars per
                # partitioned virtual layer mix outputs back into both streams.
                # Init (1, 0, 0, 1) makes this byte-identical to single-stream
                # at step 0 (modulo merge x = x_attn + x_mlp at the end).
                num_unique = len(self.blocks)
                total_virtual = self.num_loops * num_unique
                start = self.parallel_start_layer
                x_attn = x
                x_mlp = x  # value irrelevant pre-partition (we won't read it)
                split_done = False
                for _loop_idx in range(self.num_loops):
                    for layer_idx in range(num_unique):
                        block = self.blocks[layer_idx]
                        virtual_idx = _loop_idx * num_unique + layer_idx
                        if virtual_idx < start:
                            # Pre-partition: standard single-stream path.
                            x_attn = block(x_attn, x0)
                        else:
                            if not split_done:
                                # At the boundary, split the single stream into
                                # two identical copies. (x_mlp was tracking x.)
                                x_mlp = x_attn
                                split_done = True
                            partitioned_idx = virtual_idx - start
                            route = self.presid_routing[partitioned_idx].to(dtype=x_attn.dtype)
                            a2a, a2m, m2a, m2m = route[0], route[1], route[2], route[3]
                            # Attn substep reads from x_attn (resid_mix mixes
                            # x_attn with x0 inside the helper). The returned
                            # x_attn_mixed becomes the new "base" for x_attn so
                            # the resid_mix update is preserved on x_attn.
                            x_attn_mixed, attn_contrib = block._attn_substep(x_attn, x0)
                            x_attn = x_attn_mixed + a2a * attn_contrib
                            x_mlp = x_mlp + a2m * attn_contrib
                            # MLP substep reads from x_mlp directly (no
                            # resid_mix; original code did not apply resid_mix
                            # for the MLP either — it used the post-attn x).
                            mlp_contrib = block._mlp_substep(x_mlp)
                            x_attn = x_attn + m2a * mlp_contrib
                            x_mlp = x_mlp + m2m * mlp_contrib
                # Merge the two streams at the output (records' approach).
                if split_done:
                    x = x_attn + x_mlp
                else:
                    x = x_attn
...
def main() -> None:
    ...
    # 0115 PARALLEL RESIDUALS: presid_routing tensor (per-virtual-layer 4-vec)
    # lives on GPT directly, not in base_model.blocks — explicitly route to
    # scalar_params so the optimizer sees it. The substring "presid_" must
    # appear in CONTROL_TENSOR_NAME_PATTERNS (env.sh) so it stays fp32 at
    # train and quant export.
    if getattr(base_model, "presid_routing", None) is not None:
        scalar_params.append(base_model.presid_routing)
