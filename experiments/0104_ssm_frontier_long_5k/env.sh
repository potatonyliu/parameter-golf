...
export RUN_ID="0104_ssm_frontier_long_5k"
...
# 0104: SSM frontier at LONG training — the "where can we land" extrapolation point.
# Forks 0098 (kill-Mamba-2 triple-parallel + brotli + no side mem). Production batch
# 131072 (5.3x of MPS, fits 5090), canonical LR 0.045, 5000 steps.
# Total tokens trained: 5000 × 131072 = 655M tokens = ~6.5% of fineweb10B.
# Compare to 8×H100 record's 33% (3.3B tokens) — we'll see ~20% of record's training data.
# Extrapolation: if val_bpb at 5k steps trends toward 1.2-1.3, records (1.10) are
# reachable with longer training. If val_bpb plateaus at 1.5+, mechanism issue.
# Wallclock estimate: 5000 × 600ms (no compile, conservative) = 3000s = 50 min.
