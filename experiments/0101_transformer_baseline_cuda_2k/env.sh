...
export RUN_ID="0101_transformer_baseline_cuda_2k"
...
# 0101: no-ternary transformer baseline at CUDA 2000 steps. Comparator for 0099 (ternary 2k).
# Decomposes the ternary penalty at 10x training: 0099 - 0101 = ternary cost at 2000 steps.
# Same architecture as 0099 (recur+SwiGLU+mlp=8 + side memory) MINUS the BitLinear ternary body
# and MINUS the LR×3 rescue. MATRIX_LR=0.045 default (canonical), TERNARY_BODY=0.
# Step time will be slower per-step than 0099 (no ternary forward simplification but no BitLinear
# overhead either; should be similar ~250-300ms; ~10 min total).
