...
export RUN_ID="0123_transformer_anchor_h100"
...
# 0116: EMA-of-weights tier-1 port. Records use beta=0.999 with ~5-6k step training
# (-0.005 BPB gain). At our 1k screening the gain is expected small (-0.001 to
# -0.003); main purpose is to verify the technique composes with our SSM stack and
# build infrastructure for the H100 deployment recipe. Math + toy verified in
# scratch/2026-04-29_ema_{derivation.md,toy.py}. Default EMA_BETA=0.0 disables.
# 0123 TRANSFORMER-BASELINE H100 ANCHOR (Round-3 contingency, gated on Round 1+2):
# Replace kill-Mamba-2 triple-parallel with PURE ATTENTION at all 5 unique layers.
# Same: ternary body, n=5, EMA β=0.999, brotli, production batch, 600s wallclock.
# Different: ATTN_LAYER_POSITIONS=0,1,2,3,4, no PARALLEL, no MAMBA2_KILL.
# Goal: calibrate "is our gap to records' 1.10 architectural (SSM-vs-transformer)
# or stack-port-debt (no parallel-residuals, no GPTQ, no sliding-window eval)?"
# Prior anchor: 0109 pure-attn at 5090-1k landed 1.4699 (with 17.45 MB cap-bust);
# at H100 600s with EMA + ternary, predicted val_bpb in [1.20, 1.40].
# Compare to 0121 (kill-Mamba-2) and 0122 (v2 dendrocentric) at same wallclock.
export ATTN_LAYER_POSITIONS=0,1,2,3,4
export PARALLEL_LAYER_POSITIONS=
export MAMBA2_LAYER_POSITIONS=
export MAMBA2_KILL_SELECTIVITY=0
export EMA_BETA=0.999
export EMA_WARMUP_OFFSET=
# Submission-rule budget (matches 0121, 0122).
