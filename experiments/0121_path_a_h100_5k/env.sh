...
# 0121 PATH A H100 INSURANCE — best confirmed stack at H100 600s submission budget:
#   - kill-Mamba-2 triple-parallel + n=5 + ternary + brotli (0107 base)
#   - EMA β=0.999 (correct hyperparameter for ≥5k training; verified at β=0.99 in 0119)
#   - 600s wallclock (matches challenge submission budget; ITERATIONS = safe upper bound)
#   - 524288 batch (records' 8×H100 standard)
# Records anchor (track_10min_16mb/2026-03-31): 6240 steps at 96ms/step on 8×H100 SXM
# for transformer + parallel-residuals + mini-DR. Our SSM stack heavier per step (~2-3×).
# Step time prediction: 150-300ms on 8×H100; budget bounds 2000-4000 steps.
# Predicted post-quant val_bpb in [1.20, 1.45]; records best 1.1063.
#
# PARALLEL_RESIDUAL deliberately OFF — 0119 base doesn't have the code path (added in
# 0115, never merged into EMA branch). Adding requires ~30-line port. Defer to round-2.
export EMA_BETA=0.999
export EMA_WARMUP_OFFSET=
# 600s budget bounds it; ITERATIONS only an upper bound to avoid step exhaustion.
export ITERATIONS=10000
export TRAIN_BATCH_TOKENS=524288
# Warmdown ≈ 15% of expected steps (3000-4000 fit budget) → 600 steps.
export WARMDOWN_ITERS=600
export LR_WARMUP_STEPS=30
# Submission rule: 600s on 8×H100 SXM.
