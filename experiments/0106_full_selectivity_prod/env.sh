...
export RUN_ID="0106_full_selectivity_prod"
...
# 0106: full-selectivity Mamba-2 at production batch — tests whether prior session's
# "kill > full at MPS 200 steps" inverts at production scale.
# Fork from 0103 (kill at production batch, val 1.4587). Same recipe except
# MAMBA2_KILL_SELECTIVITY=0 → full Mamba-2 with input-dependent dt/B/C projections.
# Hypothesis: at MPS-200 the prior session found kill > full because selectivity was
# undertrained (5M tokens too few). At production batch + 1000 steps (131M tokens),
# the undertraining premise no longer holds; full-selectivity may overtake.
