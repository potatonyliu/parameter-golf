...
class GPT(nn.Module):
    def __init__(
        ...
        # 0092 dendritic memory (default OFF → byte-identical to 0086).
        # When DENDRITIC_MEMORY=1, build the M-dendrite frozen-pattern bank
        # from the first DENDRITIC_BUILD_TOKENS training tokens and register
        # the module. The forward path (below) adds its contribution to x
        # AFTER the embedding+bigram+rms_norm and BEFORE the first block.
        if int(os.environ.get("DENDRITIC_MEMORY", 0)):
            from modules.dendritic_memory import DendriticMemory
            dend_K = int(os.environ.get("DENDRITIC_K", 4))
            dend_M = int(os.environ.get("DENDRITIC_M", 32768))
            dend_d_content = int(os.environ.get("DENDRITIC_D_CONTENT", 32))
            dend_build_tokens = int(os.environ.get("DENDRITIC_BUILD_TOKENS", 50_000_000))
            self.dendritic_memory = DendriticMemory(
                vocab_size=vocab_size,
                K=dend_K,
                M=dend_M,
                d_content=dend_d_content,
                d_model=model_dim,
            )
            # Stream training tokens until we have enough, then call populate.
            data_path = os.environ.get(
                "DATA_PATH", "./data/datasets/fineweb10B_sp1024"
            )
            train_glob = os.path.join(data_path, "fineweb_train_*.bin")
            train_files = sorted(Path(p) for p in glob.glob(train_glob))
            if not train_files:
                raise FileNotFoundError(
                    f"DENDRITIC_MEMORY=1 but no training shards matched {train_glob}"
                )
            chunks: list[np.ndarray] = []
            accum = 0
            for f in train_files:
                t = load_data_shard(f).numpy()  # uint16
                chunks.append(t)
                accum += t.size
                if accum >= dend_build_tokens:
                    break
            train_arr = np.concatenate(chunks)[:dend_build_tokens]
            self.dendritic_memory.populate_patterns(train_arr)
        else:
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        # 0092 dendritic memory: K-gram exact-match additive residual.
        # Default OFF (self.dendritic_memory is None) → byte-identical to 0086.
        if self.dendritic_memory is not None:
            x = x + self.dendritic_memory(input_ids).to(x.dtype)
        x0 = x
        if self.recurrent:
